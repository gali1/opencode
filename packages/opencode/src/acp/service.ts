...
// ACP clients running on Windows (e.g. Zed over WSL) send a Windows-style cwd
// such as "C:\\Users\\Name" in session/new. Inside WSL that path is not
// absolute-POSIX, so it gets resolved against $HOME → "/home/user/C:\\Users\\Name",
// which does not exist and blows up SystemPrompt.environment's realPath call
// with ENOENT, surfacing as "OpenCode service failure: session". Detect a
// Windows path (drive letter or backslashes) and fall back to $HOME when it is
// not a real POSIX directory.
...
export function make(input: {
  ...
  const newSession = Effect.fn("ACP.newSession")(function* (params: NewSessionRequest) {
    params = { ...params, cwd: normalizeCwd(params.cwd) }
    const started = performance.now()
  ...
  const listSessions = Effect.fn("ACP.listSessions")(function* (params: ListSessionsRequest) {
    if (params.cwd) params = { ...params, cwd: normalizeCwd(params.cwd) }
